...
plot = ggplot(D,aes(x= as.numeric(unlist(D['Feature']))))+
  ...
  ylim(0,0.25) + ylab('Misclassification Error') + xlab('Number of Feature')
plot

data=data_train
# training Loss Plot
Loss_plot = function(n_iter=300,data,k=300,s=0.001,eta=1/nrow(data_train$y),mu=100)
{
...
Roc_plot(n_iter=300,eta=1/nrow(data_train$y),k=300,train=data_train
         ,test=data_test,mu=100, w_init= as.matrix(rep(0, ncol(data_train$x))))
